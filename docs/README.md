...
`docs/06-implementation-spec/run-flow.md` 是核心玩法流程的唯一设计源头，同时也是 MVP run 流程实现规格。其它文档涉及完整流程顺序时必须引用它，不再维护另一套流程。

## 推荐阅读顺序

### 了解项目

1. [00-overview.md](00-overview.md)：项目定位、核心流程引用、章节结构、全局设计支柱。
...
5. [06-implementation-spec/run-flow.md](06-implementation-spec/run-flow.md)：核心玩法流程、新 run、序章、第 1 章节点生成、奖励、商店、事件、存档和通关流程。
6. [02-systems/node-modifiers.md](02-systems/node-modifiers.md)：节点结束后随机事件与节点选项自带修正的设计源头。
...
| 文档 | 有效性 | 作用 |
|------|--------|------|
| [00-overview.md](00-overview.md) | 当前有效 | 项目级总览。定义游戏类型、章节结构和项目迁移决策；核心流程引用 `run-flow.md`。 |
...
| [map-encounters.md](02-systems/map-encounters.md) | 当前有效 | 地图节点候选设计源头：每步二/三选一、随机候选节点、固定步、模糊提示、节点类型。完整核心流程引用 `run-flow.md`。 |
| [node-modifiers.md](02-systems/node-modifiers.md) | 当前有效 | 节点修正设计源头：22 个节点结束后随机事件、25 个节点自带修正，以及两者的触发时机和控制方式。 |
| [roguelike-run.md](02-systems/roguelike-run.md) | 当前有效 | 奖励、成长池、金币经济和章节推进设计源头；不重复定义核心流程。 |
| [equipment.md](02-systems/equipment.md) | 当前有效 | 装备系统、稀有度、装备列表、掉落和装备管理。 |
| [potions.md](02-systems/potions.md) | 当前有效 | 药水系统、背包、使用时机、药水列表和战斗掉落入口。 |
| [shop.md](02-systems/shop.md) | 当前有效 | 商店系统设计源头：商品类型、品质分布、价格、服务、禁止项和 `shops.json` 规则。 |
...
| [run-flow.md](06-implementation-spec/run-flow.md) | 当前有效 | 核心玩法流程唯一设计源头，同时定义新游戏、序章、第 1 章随机候选节点、固定步、奖励、商店、事件、存档和章节结算的实现规格。 |
...
| 名称 | 来源文档 | 时机 | 玩家控制 | 说明 |
|------|----------|------|----------|------|
| 节点结束后随机事件 | `node-modifiers.md` | 节点完成后概率触发 | 不可控 | 22 个增益/减益（buff/debuff），属于节点修正来源之一；默认只对我方生效，明确写敌我双方的条目作为战场规则对双方生效。 |
...
- 除核心玩法流程外，系统设计意图以 `02-systems/` 为源头。
- 机器可读内容以 `03-content/data/` 为源头。
- 核心玩法流程、实现流程、字段和结算顺序以 `06-implementation-spec/` 为源头，其中完整 run 流程以 `run-flow.md` 为准。
...
| 变更类型 | 先改 | 再改 |
|----------|------|------|
| 核心玩法流程、节点结算顺序 | `06-implementation-spec/run-flow.md` | 其它文档只引用，不重复维护 |
| 地图节点候选、随机候选、固定步 | `02-systems/map-encounters.md` | `run-flow.md`、相关数据表 |
| 节点结束后随机事件或节点修正 | `02-systems/node-modifiers.md` | `run-flow.md`、机器可读节点修正表 |
...
| 商店商品、价格、服务 | `02-systems/shop.md` | `run-flow.md`、`data-schema.md`、`03-content/data/shops.json` |
...
- 不要把 `map-encounters.md` 当作旧设计；它是当前地图节点候选设计源头，但完整核心流程以 `run-flow.md` 为准。
