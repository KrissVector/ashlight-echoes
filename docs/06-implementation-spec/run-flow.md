...
- 第 1 章节点结束后随机事件池内所有事件 `weight = 100`，表示等概率抽取。
...
现有地图模块只给了事件示例，没有定死第 1 章事件列表、文案库和结果表。后续补事件内容时，必须先按该定义设计，不能把节点结束后随机事件当作事件节点替代。
...
- 第 1 章事件池非空。
- 若启用节点结束后随机事件，必须有已确认的触发概率；抽取权重必须直接读取机器池条目，不得依赖数组顺序。
