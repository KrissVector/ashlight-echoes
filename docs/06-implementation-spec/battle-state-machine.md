...
战斗药水指 `PotionDef.useTiming = "player_action"` 的药水。

```text
use_potion(potionInstanceId, selectedTargetIds):
  1. 校验阶段为 player_action
  2. 校验 potionInstanceId 存在于 RunSave.potionBag
  3. 校验 PotionDef.useTiming = "player_action"
  4. 校验目标合法；single_character 只能选择存活的我方队伍成员
  5. 从 RunSave.potionBag 移除该药水实例
  6. 创建 ActionContext
  7. 执行药水 effects
  8. 写入战斗日志
  9. 胜负检查
...
- 使用药水不消耗 AS，也不要求目标角色 `remainingAs > 0`。
- 使用药水不消耗公共能量，也不会获得能量。
- 使用药水不是出牌或普攻，不触发只监听出牌、普攻或攻击动作的触发器。
- 若药水效果需要读取 `source` 属性，`source` 默认为本次选中的目标队伍成员；例如“回复 30%最大HP”读取该目标的最大 HP。
- `duration: "battle"` 的药水增益只持续到本场战斗结束，不写回 run 状态。
